...
"""
Usage:
# Command for decoding LLM-based ASR models with multiple speech encoder:
...
from model import Speech_LLM_Zipformer_MoSE, EncoderProjector
from asr_datamodule import LibriSpeechDataset
from peft import LoraConfig, get_peft_model
from train_mose import (
    add_model_arguments,
...
def save_results(
    ...
    decode_method: str = "llm-beam-search",
):
    ...
    recog_path = (
        params.decode_dir
        / f"recogs-{test_set_name}-{decode_method}-{params.suffix}.txt"
    )
    ...
    # ref/hyp pairs.
    errs_filename = (
        params.decode_dir / f"errs-{test_set_name}-{decode_method}-{params.suffix}.txt"
    )
    with open(errs_filename, "w") as f:
        wer = write_error_stats(
            f, f"{test_set_name}-{decode_method}", results, enable_log=enable_log
        )
        test_set_wers[decode_method] = wer
    ...
    with open(errs_info, "w") as f:
        ...
        for decode_method, val in test_set_wers:
            print("{}\t{}".format(decode_method, val), file=f)

    s = "\nFor {}, WER of different settings are:\n".format(test_set_name)
    note = "\tbest for {}".format(test_set_name)
    for decode_method, val in test_set_wers:
        s += "{}\t{}{}\n".format(decode_method, val, note)
        note = ""
    logging.info(s)
...
def main():
    ...
    params.decode_dir = Path(f"{params.exp_dir}/{params.method}")
    params.suffix = (
        f"{params.method}-beam{params.beam_size}-epoch-{params.epoch}-avg-{params.avg}"
    )
