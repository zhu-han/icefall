...
class K2SpeechRecognitionDataset(torch.utils.data.Dataset):
    ...
    * ``F`` - number of features
    ...
    def __getitem__(self, cuts: CutSet) -> Dict[str, Union[torch.Tensor, List[str]]]:
        ...
        # This data is before any augmentation
        inputs_raw, _ = self.input_strategy(cuts)

        # Optional CutSet transforms - e.g. padding, or speed perturbation that adjusts
        # the supervision boundaries.
        cuts_copy = copy.deepcopy(cuts)
        for tnfm in self.cut_transforms:
            cuts = tnfm(cuts)
        for tnfm in self.cut_transforms:
            cuts_copy = tnfm(cuts_copy)

        # Get a tensor with batched feature matrices, shape (B, T, F)
        # Collation performs auto-padding, if necessary.
        inputs, _ = self.input_strategy(cuts)
        inputs_copy, _ = self.input_strategy(cuts_copy)
        ...
        for tnfm in self.input_transforms:
            inputs_copy = tnfm(inputs_copy, supervision_segments=segments)

        batch = {
            "inputs": inputs,
            "inputs_raw": inputs_raw,
            "inputs_copy": inputs_copy,
